...
conf = SparkConf().setAppName("FindTopTenCoeff")
...
triangle_per_node = flatten_edges\
...
degree_per_node = whole_edges.groupByKey().mapValues(len).sortByKey()

answer = triangle_per_node\
    .join(degree_per_node)\
    .filter(lambda x: x[1][1] >= 2)\
    .map(lambda x: (x[0], x[1][0]/(x[1][1]*(x[1][1]-1)/2)))\
